...
- **Three owner-placeable blocks** (`blocks/`) — `signal-noise/pillar-essays`, `signal-noise/sidenote` and `signal-noise/pull-quote`. All three are dynamic (`render.php`), so their text lives in block-delimiter attributes rather than serialized HTML, and all three are available on Pages as well as Notes
