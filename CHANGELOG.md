...
## [9.11.0] - 2026-06-07 — Feeds, blocks & pages
...
- **`parts/post-frontmatter.html` migrated to Block Bindings** — the reading-time and pillar slots are now bound paragraphs driven by the new `signal-noise/post-field` source (`{key:"reading_time"}` and `{key:"pillar"}`) instead of inline shortcode markup. The `wp:post-date`, `[sn_updated_date]` shortcode, the two `·` dividers, and `wp:post-terms` are byte-identical to v9.10.0; only the two bound slots changed. Empty values keep the block's fallback (no stray empty `<p>`).

### Tests

- **676 assertions across 26 suites, 0 failures** (was 610 across 20 at v9.10.0). Six new standalone CLI fixtures add 66 assertions: `tests/feed-json.php` (7 — item shape, RFC 3339 dates, JSON escaping discipline, JSON Feed 1.1 round-trip), `tests/feed-enrichment.php` (4 — Media RSS namespace, `media:content` presence/absence by featured image, plugin-guarded reading time), `tests/blocks-registry.php` (24 — `block.json` validity for both blocks, `editorScript` handle-not-`file:` trap, behavioral `render.php` output, editor-script deps, category registration), `tests/block-bindings.php` (13 — source registration, `reading_time` formatting + floor, pillar/canonical/og_title resolution + null fallbacks, unknown-key and block-context paths), `tests/colophon-template.php` (9 — `customTemplates` entry, template/pattern/footer wiring), and `tests/command-palette.php` (9 — data-island shape, bounded recent query, HTML-decoded titles, `JSON_HEX_TAG` XSS contract). Tests assert observable output, not just registration shape.
