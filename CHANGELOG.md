...
- **`/notes` hang.** Reverted the `render_block` filter's prefix-match variant introduced in 949007e back to the original exact-match `[sn_reading_time]`. The prefix-match (`[sn_reading_time` without the closing bracket) caused the front-end `/notes` index to hang server-side — server accepted the TLS handshake but never sent any response bytes within 60 seconds, while every other route (homepage, pillar, both long-forms) responded in <2s. The precise mechanism wasn't fully diagnosed under time pressure, but the revert restored `/notes` to its pre-iteration response time. The slug-attributed shortcode form `[sn_reading_time slug="..."]` still resolves correctly because it lives inside post_content (pillar cards) and is processed by WordPress core's `the_content` filter chain, which has `do_shortcode` at priority 11 — independent of this custom render_block filter. The custom filter only needs to handle the template-embedded no-args form (`[sn_reading_time]` in [page-notes.html:62](templates/page-notes.html:62)), which the exact-match catches.
