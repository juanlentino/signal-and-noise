...
### Changed
- **`/notes` centres again on wide screens.** 13.9.1 set its 1400px grid to `margin: 0` so the title would start under the header brand; on a screen wider than 1400px that pinned the grid left and left an empty band on the right (owner, 2026-09-22: "I don't like this. At all."). Back to `margin: 0 auto`. 13.9.1's gutter padding stays, so up to 1400px wide the title still starts under the brand (36 / 20 / 16px); above it the grid centres as it did before 13.9.1. `tests/layout-width-system.php` and `tests/notes-css-extraction.php` now pin `0 auto` for `/notes` and `0` for the four 760px text pages.
