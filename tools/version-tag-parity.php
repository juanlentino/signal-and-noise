 ...
 * main that cannot reach the site. In THIS repo the failure took the other documented shape: on 2026-08-20 a sweep
 * found v12.0.2, v12.0.3 and v12.0.4 all tagged onto their PRE-SQUASH branch
 * commits rather than onto main. The log subjects matched exactly, which is why
 * nobody saw it -- and `deploy.yml` is dispatched ON A TAG and checks that ref
 * out, so the tag is what ships. The trees happened to be identical; that was
 * luck, not design.
 *
 * Usage:
...
function sn_vtp_read_header( $file ) {
	...
	// The star is OPTIONAL: a plugin header is a PHP docblock (` * Version:`),
	// a theme header is a CSS comment block (`Version:`) with no star. One
	// reader for both repos, so the two copies of this tool cannot drift into
...
if ( PHP_SAPI === 'cli' && isset( $argv[1] ) && '--check' === $argv[1] ) {
	...
	$ver  = sn_vtp_read_header( $root . '/style.css' );
