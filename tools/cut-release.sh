...
#
# Theme variant. The version lives in TWO files that must never drift: style.css
# (what wp_get_theme reads) and readme.txt (Stable tag). CI already fails when
# they disagree - it did at v10.28.1 and again at v10.29.0 - so this script
# edits both rather than leaving one to a human at 2am.
#
...
CURRENT="$(grep -m1 -E '^Version:' "$STYLE_FILE" | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' || true)"
[ -n "$CURRENT" ] || die "could not parse Version: from ${STYLE_FILE}"

# The two sources of truth must already agree BEFORE a release moves them.
# Cutting on top of an existing drift would bury it inside the release diff.
README_TAG="$(grep -m1 -E '^Stable tag:' "$README_FILE" | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' || true)"
[ -n "$README_TAG" ] || die "could not parse 'Stable tag:' from ${README_FILE}"
[ "$README_TAG" = "$CURRENT" ] || die "style.css says ${CURRENT} but ${README_FILE} says ${README_TAG}. Fix the drift before cutting."
...
if [ "$DRY_RUN" -eq 1 ]; then
  ...
  echo "    ${STYLE_FILE}             Version: ${CURRENT} -> ${NEXT}"
    echo "    ${README_FILE}            Stable tag: ${CURRENT} -> ${NEXT}"
...
awk -v cur="$CURRENT" -v nextver="$NEXT" '
  !done && /^Version:/ { sub(cur, nextver); done = 1 }
  { print }
' "$STYLE_FILE" > "$tmp" && mv "$tmp" "$STYLE_FILE"
...
# one PR carries "### Fixed" twice into the changelog, the archive, and the
...
tmp="$(mktemp)"
awk -v bodyfile="$body_tmp" -v nextver="$NEXT" -v today="$TODAY" -v headline="$HEADLINE" '
  /^## \[Unreleased\]/ {
    print "## [Unreleased]"
    print ""
