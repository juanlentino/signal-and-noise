...
This file holds every entry from v1.0.0 through v12.18.3. The CURRENT release is
not here — it lives in the root file until the next cut moves it down, so no
section ever exists in both places.

It is named for the current major rather than split per major on purpose:
splitting earlier majors would rewrite history for no reader's benefit, and
nothing here is edited by hand again.
...
## [13.6.0] - 2026-09-21 — the signature and the type scale
