...
- **Coordinated release.** Two-step deploy with a brief duplication window in between:
  1. Install + activate plugin `v1.0.0`. At this point BOTH packages have the 9 modules — WP registers hooks twice (admin menu duplicated, REST endpoints last-write-wins, dashboard widgets duplicated, cron events scheduled twice).
  2. Click theme `v8.2.0` update. This removes the duplicate files + registers the action/filter listeners. Duplication window ends.
- **Duplication window behavior.** Between steps 1 and 2, the maintainer sees two *Signal & Noise* entries in the *Appearance* admin menu (one from each package). The **theme's menu entry continues to work** because theme v8.1.1's direct calls to `sn_purge_all_caches()` / `sn_self_heal_force_run()` are still in place. The **plugin's menu entry shows the same UI but the action-dispatched buttons (purge, heal) silently no-op** because theme v8.1.1 doesn't yet register the listeners. Recommendation during the window: maintainer uses the theme's menu entry, treats the duplicate as visual noise, then clicks the theme update promptly to collapse to a single working surface.
- **Reverse order is broken.** If theme `v8.2.0` ships first (without plugin yet installed), the *Signal & Noise* admin page disappears entirely and REST endpoints 404 until the plugin is installed. Public site keeps rendering. Avoid this order.
