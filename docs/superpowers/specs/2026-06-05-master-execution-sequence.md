...
- **2026-06-05 — theme v9.7.0 (on-site search) CODE-COMPLETE, RELEASE GATED.** Frontier candidate #1 built per spec+plan: `inc/search-query.php` (query-vars filter, 10 assertions; theme suite 377→387/9), `templates/search.html` (grouped Notes+Pages), header search icon, `components.css` styling. 4 feature commits on the worktree branch (`1ac1f78`→`eee4d51`), **unpushed** — push HEAD:main + bump v9.7.0 + tag on user's go. Live-verify on install (filter is front-end only). Because search took v9.7.0, the prep-minor renumbers to **v9.8.0**.
- **2026-06-05 — plugin v4.5.7 (pre-major do-now batch) COMMITTED ON MAIN (local), RELEASE GATED.** Items 1+2 = patch: `/sn-login` X-Robots-Tag noindex header (`inc/login-hide.php` + `tests/login-noindex-header.php`, 6 assertions) + 22-inline-style consolidation into `admin.css` (fixes the brand-red wp-admin leak → native `#2271b1`, dedups 40/20/40). Items 3+4 = docs no-bump: `sn_`/`snt_` prefix convention in theme `WORDPRESS-REFERENCE.md` §10 (commit `f48458c`, on theme worktree branch) + plugin CHANGELOG Mimestream backfill of 5 legacy entries. 4 plugin commits (`4f3b796`→`a607e62`) **unpushed** — push origin main + tag v4.5.7 on user's go. All 31 plugin suites green; PHPCS 0/0. The bigger v4.6.0 feature build + its stale-baseline reconciliations remain parked for the v5.0.0 BC.
