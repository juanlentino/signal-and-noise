...
1. **Owner installs:** theme **v9.15.5** + plugin **v4.14.5** via wp-admin → Dashboard → Updates (tag pushes don't auto-deploy). Supersede v9.15.4 / v4.14.3. *(Plugin v4.14.5 supersedes my v4.14.4 — see addendum.)*
...
## State
- Both repos: 0 open PRs. Theme at tagged HEAD v9.15.5; **plugin now at v4.14.5** (a sibling session's resolver fix landed cleanly on top of my v4.14.4 — addendum). My worktrees (`/tmp/snt-fix`, `/tmp/snt-audit`) removed; merged branches deleted. Pre-existing `.claude/worktrees/*` and the stale leftover branches `claude/info-hardening` (theme, = shipped v9.15.4) + `claude/ssrf-url-validation` (plugin, = shipped v4.14.1) are squash-merge leftovers — safe to prune. The bare plugin checkout at `~/Projects/signal-and-noise-tools` remains stale at v4.7.0 (known trap — always audit/branch off `origin/main`).

## Addendum (post-session reconciliation)
While verifying "anything else outstanding," found a **concurrent session shipped plugin v4.14.5** (`40ee65c`, tagged) — the `[sn_reading_time slug]` shortcode-resolver fix (the INFO #4 above), the *same* reading-time oracle I closed theme-side, fixed at the resolver layer. Composition verified clean: linear history `v4.14.4 → v4.14.5`, my login-hide fix intact, theme untouched at v9.15.5, 0 open PRs.
- **Benign interaction:** my theme ability (v9.15.5) allows an editor to read *their own* draft's time (`current_user_can('read_post')` branch), but the v4.14.5 resolver returns empty for *any* non-public post → an editor now gets `minutes=0` too. More restrictive, **not a security gap**. My theme test's "authorized reader → real time" assertion passes via a stub but is end-to-end-inaccurate for that edge case.
- **Optional reconciliation (NOT urgent, coordinate with the active sibling session):** align the theme ability's `read_post` branch to the resolver's public-only policy (drop the read_post allowance + assert editor→0), or document the divergence. No v9.15.6 shipped here — benign issue + a sibling session is active.
