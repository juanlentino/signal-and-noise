...
1. **Owner installs:** theme **v9.15.6** + plugin **v4.14.5** via wp-admin → Dashboard → Updates (tag pushes don't auto-deploy). These are the latest tagged HEADs. *(v9.15.6 = reconciliation, v4.14.5 = resolver fix — see addendum.)*
...
4. **~~Optional follow-up (INFO, not shipped):~~ SHIPPED by a concurrent session as plugin v4.14.5** — see addendum. The `[sn_reading_time]` shortcode resolver (`inc/reading-time.php`) now returns empty for non-public posts.

## State
- **Theme at v9.15.6, plugin at v4.14.5** (both tagged HEADs). 0 open PRs either repo. The bare plugin checkout at `~/Projects/signal-and-noise-tools` remains stale at v4.7.0 (known trap — always audit/branch off `origin/main`).

## Addendum (post-session reconciliation + cleanup — DONE)
**1. Concurrent plugin v4.14.5.** A sibling session shipped plugin **v4.14.5** (`40ee65c`, tagged) — the `[sn_reading_time slug]` shortcode-resolver fix (the INFO #4 above), the *same* reading-time oracle I closed theme-side, fixed at the resolver layer. Composition verified clean: linear history `v4.14.4 → v4.14.5`, my login-hide fix intact, 0 open PRs.

**2. Reconciliation SHIPPED — theme v9.15.6 (`#7`, merged + tagged).** The v4.14.5 resolver is public-only (returns empty for any non-public post regardless of cap), so my v9.15.5 ability's `current_user_can('read_post')` branch was **dead and divergent** (it surfaced the wrapper's 5-min fallback to an editor, and my test asserted a real time that only held via the stub). Made `get-reading-time-for-slug` **public-only** to match the resolver — gate on `is_post_publicly_viewable()` alone; non-public → uniform `minutes=0`. TDD (assertion flipped to expect 0 → RED → public-only gate → GREEN); added the missing `is_post_publicly_viewable` stub to the registration harness. 103 integration assertions / 29 suites green; PHPCS falsification-verified. Security outcome unchanged, now enforced identically by both layers. (Contrast `get-active-template-structure`: in-theme, still honors `read_post` — policy difference justified by dependency difference.)

**3. CI: `claude-review --max-turns 15 → 30` (`c308ce7`, on main).** PR #7's house-rules reviewer hit `error_max_turns` twice (zero findings posted — a tooling crash, not a finding) because 15 turns is too tight when it chases the cross-repo plugin context referenced in the diff. Bumped to 30 on main (byte-match requirement), synced the PR branch, review went green, then merged. Not a code issue — all required checks + `security` were green throughout.

**4. Worktree/branch pruning — DONE.** Plugin: removed 8 stale worktrees + branches (v4.8.0–v4.13.1, anthropic-tooling, etc.) + the `ssrf-url-validation` remote branch → only `main` remains. Theme: removed 20 clean worktrees + 24 merged branch refs + 4 merged remote branches + the `info-hardening` remote branch.
- **KEPT (unmerged branch refs, worktrees already removed):** `claude/anthropic-tooling` (4 unique commits — handoff says merged, but cherry shows unique; not deleted on the ambiguity), `claude/hungry-swirles` (1).
- **KEPT (worktrees with genuine UNCOMMITTED changes — your call to keep or discard):**
  - `claude/recursing-wilbur-019e25` (`364dee7`) — modified `CHANGELOG.md` + 3 untracked `_claude/notes/2026-05-15-contact-*.md` drafts.
  - `claude/zen-mahavira` (`de7d9ac`, an ancient base) — substantial uncommitted edits (`functions.php`, `style.css`, CSS, `composer.json`, `readme.txt` + untracked `inc/`).
  These two refused auto-removal (`git worktree remove` correctly blocks on uncommitted files); I did **not** `--force` them. Inspect or `git worktree remove --force` when you've confirmed nothing's wanted.
