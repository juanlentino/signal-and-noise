...
> **Corollary: there are no global `render_block` shortcode bridges any more (#389).** Eleven modules used to keep `add_filter( 'render_block', ..., 10, 2 )` re-resolving a token in every rendered block, "belt-and-suspenders" since 9.9.1. On the template path they were never load-bearing: core `do_shortcode`s the raw markup first (above). In post content the order is reversed, `do_blocks` at `the_content` priority 9 and `do_shortcode` at 11, so a bridge there resolved a token two filters early with the same bytes `do_shortcode` would have produced. `render_block` runs only inside `do_blocks()`, so it cannot change what the block editor paints either (the `core/shortcode` edit component renders the token in a textarea on the client). Since 13.1.0 the furniture is PHP-only blocks and no template carries those tokens, and the footer year is a `signal-noise/site-field` binding (`inc/block-bindings.php`, `parts/footer.html`). **Verified 2026-06-07:** `[sn_post_pillar]` in `parts/post-frontmatter.html` resolved on the live front end with no bridge of its own (0 literal-token leaks). Two bridges survive on purpose and `tests/render-block-bridges.php` names them: `inc/404-recovery.php` until #386 places the 404 block, and `inc/reading-path-slot.php`, whose plugin-absent guard (an empty slot instead of a literal `[sn_reading_path]` in prose) is load-bearing until the plugin ships `signal-noise/reading-path`. Do not add a new one: a shortcode in template markup resolves without it, and a token that must survive the editor canvas wants a block or a binding.
...
**Cron-arg identity trap (added v10.25.0).** WP keys a scheduled event by `md5(serialize($args))`, so the args are part of the event's identity. `wp_next_scheduled($hook)` with no args will NEVER match an event scheduled with `wp_schedule_single_event($ts, $hook, array($x))`; you must query with the SAME args. This matters when you carry a payload on the event (e.g. the render epoch a purge bumped to). To "replace, don't stack" across payloads, `wp_clear_scheduled_hook($hook)` is the WRONG tool: it only clears events whose args hash to the empty-args key, leaving older-payload events behind. Use `wp_unschedule_hook($hook)` (WP 4.9+), which drops every event for the hook regardless of args. Reference implementation: `sn_schedule_auto_purge_verify()` in [inc/purge-verify-cron.php](../inc/purge-verify-cron.php) is idempotent for the same epoch (`wp_next_scheduled` WITH the arg) and replaces for a newer epoch (`wp_unschedule_hook` then reschedule), so rapid successive auto-purges collapse to a single pending verify targeting the latest render.
...
The theme is presentation; the companion plugin [`signal-and-noise-tools`](https://github.com/juanlentino/signal-and-noise-tools) holds operational tooling. They communicate via 9 theme-listener WP hooks (3 since v8.4.0; +1 in v9.1.6/plugin v4.1.1; +3 accreted in v10.13.0/v10.38.0/v10.39.0; +1 in v10.43.0 against the plugin's v9.54.0 error seam — all first tabled in the v10.49.0 doc sweep; +1 in v10.51.1 against the plugin's v9.88.0 robots seam, tabled in v11.4.5). **The split is complete as of v8.4.0 / Tools v1.3.0 — no further migrations are planned.** See `docs/superpowers/specs/2026-05-15-companion-plugin-phase-1-design.md` and successors for the migration history.
...
**Contract hooks — 9 cross-package hooks the theme LISTENS on (3 added v8.4.0–v8.5.0; +1 added v9.1.6; +3 added v10.13.0–v10.39.0; +1 added v10.43.0, tabled in the v10.49.0 doc sweep; +1 added v10.51.1, tabled in v11.4.5). All 9 are pinned by [tests/cross-package-listeners.php](../tests/cross-package-listeners.php):**

| Hook | Type | Dispatched by | Listened by |
| --- | --- | --- | --- |
| `sn_purge_all_caches_result` | filter | plugin: `apply_filters( 'sn_purge_all_caches_result', 0, $args )` returns int count. `$args` shape: array with optional keys `template_overrides` (bool, default true), `cloudflare` (bool, default true), `object_cache` (bool), `sn_transients` (bool), `origin_html` (bool), `repopulate` (bool), `verified` (bool, default false — v10.23.0: routes the CF leg through the blocking `sn_cf_purge_everything_verified()` accept-confirmation and writes `sn_last_purge_report`; the manual "Purge All Caches"/"Full reset" buttons pass it, fast auto-purges do not), `reason` (string for diagnostic logging). Theme listener applies `wp_parse_args` against `sn_purge_all_caches()` defaults; unrecognized keys are silently ignored. **Return is still int** — diagnostics ride the `sn_last_purge_report` option side-channel, not the return shape. | theme: [`inc/template-maintenance.php`](../inc/template-maintenance.php) wraps `sn_purge_all_caches()` |
...
- `sn_after_full_cache_flush` action — theme dispatches at the end of `sn_purge_all_caches()` with `( $args, $cleared )`; the theme's own `inc/purge-verify.php` listens (writes `sn_last_purge_report`), and it remains available for plugin extension.
- `sn_before_cache_flush` action (v10.23.0) — theme dispatches at the START of `sn_purge_all_caches()` with `( $args )`, symmetric with `sn_after_full_cache_flush`; `inc/purge-verify.php` hooks it to bump the render epoch before the legs fire.
- `sn_last_purge_report` option (v10.23.0; +Tier-2 fields v10.24.0) — theme writes (autoload=no; NOT a transient, because the purge deletes `_transient_sn_%`), plugin reads via `get_option()` to render the "Caches" glance card's last-purge line. Shape: `{ time, mode: 'verified'|'auto', epoch, legs: { breeze_file: bool, varnish: {via,ok,http,operation_id}|{via,listener}, cf: {accepted,http,cf_success}|{dispatched,confirmed} }, template_overrides }` plus, on a **verified** purge only (v10.24.0): `routes: [{ url, http, epoch_seen:int|null, fresh:bool|null, cf_cache_status }]` + `resolved: bool`. The plugin surfacing "N/N routes fresh" from `routes[]` is a pending follow-up.
- `sn_verified_purge_routes` filter (v10.24.0) — the cache-critical routes the verified purge probes (default `/`, `/notes/`, `/provenance/`). `sn_purge_verify_backoff_us` filter (v10.24.0) — escalation backoff µs (default 1.5s). Both in `inc/purge-verify.php`; the box→CF reachability that makes the box-direct probe viable is confirmed (verified-purge spec §12).
- `sn_render_epoch` option + `<meta name="sn-render-epoch">` (v10.23.0) — a monotonic per-render marker emitted on `wp_head`, bumped at the start of every edge-affecting purge; the plugin's freshness dot compares it canonical-vs-cache-busted to detect a stale edge (a universal signal alongside the combined-CSS hash).
...
| 37 | **Install-time hooks (`upgrader_process_complete`, `upgrader_post_install`, etc.) cannot self-observe.** `WP_Upgrader::upgrade()` runs in the SAME PHP request that started the upgrade. Whatever PHP code was loaded at request-start (the OLD plugin version) is what's in memory when `do_action( 'upgrader_process_complete', ... )` fires. WordPress does NOT re-`require` plugin files mid-upgrade — the new files exist on disk after `install_package()` but are NOT yet in memory. **A new handler shipping in version N cannot observe the install that brings version N into existence — only future N → N+1 installs.** Plugin v4.1.4 shipped a new `upgrader_process_complete` handler to record wp-admin Updates into the Recent Deploys panel; the v4.1.3 → v4.1.4 install was missed because v4.1.3's PHP had no handler registered. Result: dashboard stayed stuck at v3.8.6 (the last GHA-auto-deploy) until v4.1.5 added the self-heal. | `wp-admin/includes/class-wp-upgrader.php` `WP_Upgrader::run()` (calls `install_package` then `do_action('upgrader_process_complete', $this, $hook_extra)` in the same request context) | Pair EVERY install-time hook handler with an `admin_init` version-check that self-heals the bootstrap gap. Compare in-memory plugin/theme versions to a tiny autoloaded sentinel option (`wp_load_alloptions` cache → in-memory hot path); on diff, scan history via `has_version` helper to dedupe vs. the install hook, then record + update sentinel. Plugin v4.1.5 [`inc/deploy-history.php`](https://github.com/juanlentino/signal-and-noise-tools/blob/main/inc/deploy-history.php) ships this pattern. **General rule:** if the bootstrap question is "but when does THIS version get recorded?" — your install hook can't answer it. Use admin_init self-heal. |
| 39 | **A DYNAMIC block's `render_callback` does NOT receive `source:html`/`source:text` attributes server-side — they arrive EMPTY.** WordPress populates `$attributes` for a dynamic block from ONLY the block's comment-delimiter JSON (`$parsed_block['attrs']`) plus `block.json` defaults. Attribute *sourcing from saved inner HTML* (the `source`/`selector` mechanism) runs **client-side only**, in the JS editor (`@wordpress/blocks` `getBlockAttributes`). So a block declaring `"content": {"type":"string","source":"html","selector":".x"}` + a `render.php` reading `$attributes['content']` renders **empty content** on the front end — the user's text lives in the saved innerHTML, which a dynamic block ignores in favor of its render callback. A CLI/PHP test that hand-injects `$attributes` into `render.php` is a FALSE-GREEN: it stubs away the exact gap. Theme v9.11.0 first shipped both new blocks broken this way; the adversarial review caught it (the 26-suite green run did not). | `wp-includes/class-wp-block.php` (`WP_Block::__construct`: `$this->attributes = $this->parsed_block['attrs'] ?? []`; `WP_Block_Type::prepare_attributes_for_render()` fills defaults only — no HTML sourcing) | For a dynamic block that needs editable rich text, make the attribute **plain** (`{"type":"string"}`, no `source`/`selector`) so its value persists in the comment-delimiter JSON and is populated in `render.php` server-side, and have `save()` return `null` (fully dynamic, no stored markup → no block-validation/recovery churn). RichText in `edit()` works fine against a plain attribute. Lock it with a test asserting no block attribute carries a `source` key. Theme v9.11.0 `blocks/{sidenote,pull-quote}/block.json` + `blocks/editor.js` ship this. **General rule:** `source:html` is editor-hydration sugar; never read it from `$attributes` in a render callback. |
| 38 | **Hook-suffix substring guards (`strpos`) silently break across IA reorgs.** When a plugin scopes a JS enqueue to a specific admin sub-page via `if ( strpos( $hook_suffix, 'sn-cron' ) === false ) return;`, the guard depends on the registered submenu slug appearing in the WordPress-generated `$hook_suffix` (e.g., `signal-noise_page_sn-cron`). If a later IA reorg consolidates the page into a sub-tab of a parent page (registered with a different slug), the `$hook_suffix` becomes `signal-noise_page_sn-automation` and the substring check returns FALSE silently. The JS never loads. No PHP error, no `_doing_it_wrong`, no test failure (tests guard the dispatcher, not the asset graph). Cron-dashboard JS was unloaded for 5 months between v3.8.1's IA reorg and v4.1.6's D-11 audit fix. | `wp-admin/includes/plugin.php` `add_menu_page` / `add_submenu_page` (hook-suffix generation) | Use `in_array( $hook_suffix, sn_admin_page_hooks(), true )` against a centrally-collected list of registered hooks instead. The list is the canonical source of truth — IA changes update the list, the guards follow. JS that gates internally on selectors (no-op if its targets aren't on the page) is safe to load on broader sets of pages. Plugin v4.1.6 [`inc/cron-dashboard-admin.php`](https://github.com/juanlentino/signal-and-noise-tools/blob/main/inc/cron-dashboard-admin.php) replaces the strpos guard with the canonical `in_array(sn_admin_page_hooks())` pattern from [`inc/admin-page.php:532`](https://github.com/juanlentino/signal-and-noise-tools/blob/main/inc/admin-page.php). **General rule:** every substring/regex match against a framework-generated identifier is a latent IA-reorg booby trap. Match against a registry, not a string. |
| 40 | **A `template_redirect` virtual route for a path with no backing post returns HTTP 404 (with its body attached) unless the handler calls `status_header( 200 )`.** `$wp->main()` runs `handle_404()` → `status_header(404)` + `nocache_headers()` then `send_headers()` BEFORE `do_action('template_redirect')` fires (template-loader.php includes after `wp()` returns). For a postless path `$wp_query->posts` is empty, so `handle_404()` commits the 404; a later `header('Content-Type: …')` + echo in the handler does NOT reset the status. The **/notes** virtual route escapes this only because `/notes` resolves to a real published Page (200 already set). A status-aware crawler/IndieWeb tool treats a 404 body as "does not exist," so an advertised resource (e.g. `<link rel="author" href="/humans.txt">`) effectively vanishes; `nocache_headers` also defeats edge caching. **Test trap:** a suite that exercises only the body-builder + route-matcher is a false-green — it never invokes the serve handler, so the missing 200 ships green. | [`wp-includes/class-wp.php`](https://raw.githubusercontent.com/WordPress/WordPress/master/wp-includes/class-wp.php) `handle_404()` + `main()`; `wp-includes/template-loader.php` (`do_action('template_redirect')`) | In the serve handler, `if ( function_exists('status_header') ) status_header( 200 );` BEFORE the Content-Type header + echo. Add a behavioral test invoking the serve path that asserts a stubbed `status_header` recorded 200 (not just the body string). Theme v10.5.0 [`inc/humans-txt.php`](../inc/humans-txt.php) `sn_humans_txt_send()` does this — caught by the cluster-2 adversarial review, not the green 20-suite run. |
| 41 | **A status-code-only link probe misreads a Cloudflare bot challenge as link rot.** A Cloudflare-gated host (academic publishers like SSRN, and many others) answers an automated HEAD/GET from a non-browser client with an `HTTP 403` (or `503`) bot-challenge interstitial carrying the header `cf-mitigated: challenge`. The resource is LIVE: a human in a browser solves the challenge and reaches it. A link checker that classifies `ok = (200..399)` files this as a dead link. Both Health-tab probes did so: the external link-rot check flagged two live SSRN citations (the reported bug), and the internal broken-links check shared the same blind spot. `cf-mitigated` is set by Cloudflare ONLY on responses it generated itself, so keying on it never masks an origin `4xx` merely passed THROUGH Cloudflare. Watch-out: a self-declared bot UA from a datacenter ASN can draw a harder response (a hard block, not an interactive challenge) than a browser UA, so confirm the SERVER (not your laptop) receives `cf-mitigated: challenge` before assuming the skip applies. | Cloudflare edge behavior, not a WP-core file (the `cf-mitigated` response header, value `challenge`), surfaced through `wp_remote_retrieve_headers()` | Read the response headers, not just the code. Shared classifier `sn_health_is_bot_challenge( $code, $headers )` in [`inc/health-probe-classify.php`](https://github.com/juanlentino/signal-and-noise-tools/blob/main/inc/health-probe-classify.php) skips a `403`/`503` carrying `cf-mitigated: challenge` (plugin v6.48.3 external probe, v6.48.4 internal probe + the shared extraction). Lock it with a test asserting a PLAIN `403` (no `cf-mitigated`) still rots, so the fix cannot decay into 'ignore all 403s'. |
| 42 | **`wp_remote_retrieve_headers()` returns a `CaseInsensitiveDictionary` (ArrayAccess), not a plain array.** In production the header bag is `WpOrg\Requests\Utility\CaseInsensitiveDictionary`: `isset($h['Cf-Mitigated'])` and `$h['cf-mitigated']` resolve case-insensitively via `offsetExists`/`offsetGet`, and a multi-value header arrives joined into one string. A standalone test that stubs the header bag as a PLAIN PHP `array` exercises a DIFFERENT branch than the live site (the `is_array()` `foreach` vs the object's `ArrayAccess` path), so a header-reading classifier can pass every test and still misbehave in production. A v6.48.3 review caught exactly this coverage gap. | `wp-includes/class-wp-http.php` (response `headers` stored as `WpOrg\Requests\Utility\CaseInsensitiveDictionary`); `wp-includes/Requests/src/Utility/CaseInsensitiveDictionary.php` | Handle BOTH shapes in any header reader (an `is_array()` branch plus an `$h instanceof ArrayAccess` branch), and drive the test through a faithful `ArrayAccess` stand-in with lowercased keys, not only a plain array. [`tests/health-probe-classify.php`](https://github.com/juanlentino/signal-and-noise-tools/blob/main/tests/health-probe-classify.php) ships the `SN_CI_Headers` stand-in. |
| 43 | **`wp_register_ability()`'s `output_schema` is VALIDATED against the execute result at runtime, and every test bypasses it.** The schema is a second copy of the ability's contract, and the copy production actually enforces. A test that calls the `execute_callback` directly (which is how all of ours are written) never goes through registration, so a payload can drift from its own declaration and stay green forever. Theme v12.0.0 reshaped `get-design-tokens`' `colors` from a flat `slug => hex` map into a struct while the schema still said every property is a hex string. The ability failed the moment the release installed, behind 2,349 passing assertions. | Abilities API validation layer (WP 7.0+) | **Grep the `output_schema` in the same commit that changes a payload's shape.** Guard it: read the schema from the REGISTERED ability (invoke the real registrar, capture via a `wp_register_ability` stub) and validate the real payload against it, negative-controlled against the actual defect. `tests/design-tokens.php` does this. Note the masking: `sn-site-facts` degrades ONE failing fact to `{error:"unavailable"}` and returns its siblings normally, so nothing goes red and only asking for that fact by name reveals it. |
| 44 | **WordPress generates unguarded `:hover` rules from `theme.json` `elements.link:hover` / `elements.button:hover` into `global-styles-inline-css`.** `theme.json` cannot express a media query, so there is no way to make them touch-aware at the source. Guarding only the theme's own hover rules makes core's the ones that apply on a phone, where a tap applies `:hover` and keeps it until the next tap elsewhere. Theme v12.0.3 guarded all 70 theme rules and thereby widened the gap: 5 unguarded `:hover` rules were still being served, all core's. | `wp-includes/global-styles-and-settings.php` (theme.json to CSS compiler) | A `@media (hover: none)` block restoring the resting values. Core emits `:root :where(…)`, which is **(0,1,0)** because `:where()` contributes nothing, so a plain `:root`-prefixed selector wins without `!important`. Verify by counting `:hover` rules in the SERVED stylesheets, not the repo's: a repo-only sweep reported "0 unguarded" while 5 were live. |
| 45 | **`color-scheme: dark` does not propagate into a cross-origin iframe, and the browser may paint that iframe's canvas white.** When the embedded card has rounded corners and the theme squares the iframe off, the white canvas shows through at the four corners. Whether it does is browser-dependent and not something the page can guarantee. | CSS `color-scheme` / iframe canvas painting, not a WP layer | Give the iframe an explicit background that matches whatever it sits on, so the corners blend **by construction**: the parent's color when the embed is inside a card (`.sn-music-featured__player` uses `asphalt`), or a fixed dark backdrop when it sits on the page (`--sn-embed-backdrop`, deliberately identical in both schemes because it is matching somebody else's chrome). Do not reach for the fixed backdrop inside a card: black notches against a `#171717` box are not an improvement on white ones. |
