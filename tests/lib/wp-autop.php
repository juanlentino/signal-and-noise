<?php
/**
 * WordPress core's wpautop(), verbatim, for the standalone suites.
 *
 * NOT a suite - tests/run.sh globs tests/*.php non-recursively, so nothing in
 * tests/lib/ is swept. tests/404-recovery.php requires this to pin, byte for
 * byte, that the signal-noise/suggestions-404 block renders what the
 * core/shortcode block rendered from the same template slot (#386). The
 * house suites stub wpautop() with a marker wrapper, which proves the wrapper
 * is applied but cannot prove two inputs collapse to the same bytes; only the
 * real algorithm can.
 *
 * Copied unchanged from WordPress 7.1, wp-includes/formatting.php
 * (WordPress/WordPress at tag 7.1, commit b998fef9238a): wpautop() lines
 * 446-605, wp_html_split() 615-617, get_html_split_regex() 626-673,
 * wp_replace_in_html_tags() 757-797, _autop_newline_preservation_helper()
 * 808-810. Every function is guarded so a suite that already stubs one keeps
 * its stub. Re-copy, never edit: a local change here would make the pin
 * measure this file instead of core.
 */
