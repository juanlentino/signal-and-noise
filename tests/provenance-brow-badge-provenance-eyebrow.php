...
echo "\nGroup: shape C — the essays' .sn-provenance-eyebrow is a brow too\n";
// /provenance/over-detection opens with this exact line inside its hero group.
$eyebrow = '<p class="sn-provenance-eyebrow has-blood-color has-text-color has-body-font-family">A short read · 5 min read</p>';
$GLOBALS['__content'] = '<!-- wp:paragraph {"className":"sn-provenance-eyebrow"} -->' . $eyebrow . '<!-- /wp:paragraph --><!-- wp:heading {"level":1} --><h1>Music has a verification problem.</h1><!-- /wp:heading -->';
$h1 = '<h1 class="wp-block-heading font-display sn-provenance-title">Music has a verification problem.</h1>';
$out_h = sn_prov_brow_heading_filter( $h1, array( 'attrs' => array( 'level' => 1 ) ) );
ok( $out_h === $h1, 'the h1 filter leaves an essay alone: its content carries an authored brow, which the paragraph filter owns' );
ok( ! sn_prov_brow_placed(), 'and records no placement' );
$out = sn_prov_brow_filter( $eyebrow );
ok( false !== strpos( $out, 'Verified v1' ), 'the badge joins the provenance eyebrow' );
ok( false !== strpos( $out, 'sn-prov-brow-sep' ), 'as a second segment, with the middot separator' );
ok( strpos( $out, 'A short read' ) < strpos( $out, 'sn-prov-brow' ) && substr( $out, -4 ) === '</p>', 'inside the same paragraph, after the reading time, before the closing tag' );
ok( sn_prov_brow_placed(), 'placement recorded, so the plugin foot append stands down (the badge no longer lands last on the essays)' );
ok( sn_prov_brow_filter( $eyebrow ) === $eyebrow, 'a second eyebrow paragraph is left alone: one brow per page' );
