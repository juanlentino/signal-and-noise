...
// The survivor, with the change that retires it (#386 retired 404-recovery.php's
// bridge when signal-noise/404-suggestions took the templates/404.html slot).
$survivors = array(
...
sort( $found );
ok( array_keys( $survivors ) === $found, 'the only global render_block filter in inc/ is the named survivor (found: ' . implode( ', ', $found ) . ')' );
...
// Nothing is left in template markup for a bridge to find: the survivor's
// token and the plugin's [sn_reading_time slug] eyebrows, and no other.
$tokens = array();
...
ok( array( 'sn_reading_path', 'sn_reading_time' ) === $tokens, 'template markup carries only the survivor\'s token and the plugin\'s reading-time eyebrow (found: ' . implode( ', ', $tokens ) . ')' );
